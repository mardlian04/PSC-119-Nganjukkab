                                ...
                                <tr class="group hover:bg-red-50/30 transition-colors duration-200">
                                    ...
                                    <td class="px-8 py-5">
                                        <div class="max-w-xs md:max-w-md">
                                            <span
                                                class="block text-sm font-bold text-gray-800 truncate">{{ $g->judul }}</span>
                                            <span
                                                class="block text-[11px] text-gray-400 mt-0.5 truncate">{{ $g->deskripsi ?? 'Tidak ada deskripsi.' }}</span>
                                        </div>
                                    </td>
                                    ...
                                    <td class="px-8 py-5">
                                        <div class="flex items-center justify-end gap-3">
                                            ...
                                            <form method="POST" action="{{ route('gallery.destroy', $g->id) }}"
                                                onsubmit="return confirm('Hapus foto ini permanen?')" class="inline">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
                                ...
                                <tr>
                                    <td colspan="4" class="px-8 py-20 text-center">
                                        <div class="flex flex-col items-center">
                                            ...
                                            <p class="text-gray-500 font-medium">Belum ada koleksi foto.</p>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
